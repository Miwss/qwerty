...
public class SLova {
    public static boolean[][] crossword1 = new boolean[45][45];
    public static void main(String[] args) {
        ...
        detectAllWords(crossword, "INTERRUPTING", "GRAMMAR", "WISHING");
        /*
Ожидаемый результат
home - (5, 3) - (2, 0)
same - (1, 1) - (4, 1)
    ...
    public static List<Word> detectAllWords(int[][] crossword, String... words) {
        ...
        for (String word1 : words ){
            ...
            for (int j = 0; j < 45; j++)
            {
                ...
                for (int i = 0; i < 45; i++)
                {
                    char m1 = (char) crossword[i][j];
                    if (chars[0] == m1) {word.setStartPoint(j,i);startword = true; k = crossword1[j][i] = !crossword1[j][i]; break;}
                }
                if (startword && k) {break;}
            }
            ...
            for (int j = 0; j < 45; j++)
            {
                ...
                for (int i = 0; i < 45; i++)
                {
                    ...
                    char m = (char)crossword[j][i];
                    if ((chars[chars.length-1] == m))  {word.setEndPoint(i,j); k1 = crossword1[j][i] = !crossword1[j][i];endword = true; break;}
                }
                if (endword && k1) {break;}
            }
            list.add(word);
